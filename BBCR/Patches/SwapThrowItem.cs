...
namespace BBCR.Patches
{
    ...
    class SwapThrowItem
    {
        ...
        {
            if (__instance.held && !__instance.clickBuffer && InputManager.Instance.GetDigitalInput("UseItem", onDown: true) && ModdedOptionMenu.SpawnThrowItemConfigEnabled)
                __instance.Throw();
        }
        ...
        {
            if (InputManager.Instance.GetDigitalInput("Interact", onDown: true) && ModdedOptionMenu.SpawnThrowItemConfigEnabled)
                return false;
